...
export function computeVisibleNodes(filters: FilterState): VisibleNodes {
  // Filter tasks
  ...
  const visibleSubAgents = SUB_AGENTS.filter((sa) =>
  ...
  visibleAgents.forEach((agent) => {
    const subAgents = visibleSubAgents.filter((sa) => sa.parentId === agent.id);
